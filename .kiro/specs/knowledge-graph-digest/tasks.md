...
- [x] 1. 数据模型与枚举定义
  - [x] 1.1 在 `backend/app/models/enums.py` 中追加所有新增枚举类型
  ...
  - [x] 1.2 创建 `backend/app/models/knowledge_graph.py`，定义知识图谱 SQLModel 表
  ...
  - [x] 1.3 创建 `backend/app/models/curriculum.py`，定义课程结构 SQLModel 表
  ...
  - [x] 1.4 在 `backend/app/core/database.py` 中注册新模型，确保 create_all 能创建所有新增表
...
- [x] 2. 基础设施工具层
  - [x] 2.1 在 `backend/app/core/exceptions.py` 中追加新增异常类
  ...
  - [x] 2.2 在 `backend/app/utils/` 中实现工具函数
  ...
  - [x] 2.7 实现 pending → active 批量激活集中 helper
...
- [x] 4. 数据访问层
  - [x] 4.1 创建 `backend/app/repositories/kg_repo.py`，实现知识图谱 Repository
...
- [x] 5. LLM 抽取层
  - [x] 5.1 创建 `backend/app/agents/digest/kg_extractor.py`，实现候选知识抽取
    - 定义 Pydantic 模型：CandidateNode、CandidateEdge、ChunkExtractionResult
    - 实现 extract_candidates 异步函数，使用 Instructor 结构化输出
    - _Requirements: 5.1, 5.2, 5.3, 5.4_

  - [x] 5.2 创建 `backend/app/agents/digest/prompts/kg_prompts.py`，编写候选抽取 prompt
  ...
  - [x] 5.5 创建 `backend/app/agents/digest/kg_clusterer.py`，实现批内候选聚类
...
- [x] 6. 对齐层
  - [x] 6.1 创建 `backend/app/agents/digest/kg_resolver.py`，实现节点对齐（Entity Resolution）
  ...
  - [x] 6.3 在 `kg_resolver.py` 中实现边对齐（Relation Resolution）
...
- [x] 7. 影响集分析
  - [x] 7.1 创建 `backend/app/agents/digest/kg_impact_analyzer.py`，实现影响集分析器
...
- [x] 8. GraphDigestJob 工作流
  - [x] 8.1 创建 `backend/app/agents/digest/kg_workflow.py`，实现 KG 工作流状态机
  ...
  - [x] 8.2 编写构建进度单调递增属性测试（P0）
    - **Property 10: 构建进度单调递增**
    - **Validates: Requirements 8.4**

  - [x] 8.3 编写文件范围限定属性测试（P1）
    - **Property 11: 文件范围限定**
    - **Validates: Requirements 8.6**

- [x] 9. 图谱服务层与 API
  - [x] 9.1 创建 `backend/app/services/knowledge_graph_service.py`，实现图谱服务（Phase 1 部分）
  ...
  - [x] 9.2 创建 `backend/app/schemas/knowledge_graph.py`，定义 API Schema（Phase 1 部分）
  ...
  - [x] 9.3 扩展 `backend/app/api/knowledge.py`，新增图谱路由
  ...
  - [x] 9.4 在 `backend/app/main.py` 中注册新路由
    - _Requirements: 12.1_

- [x] 10. 向量检索兼容性保障
  - [x] 10.1 确认增量构建过程中继续为每个 DocumentChunk 生成 embedding 并写入 chunk_embeddings
    - 确认现有 vector_search 函数接口签名和行为不变
    - 确认 Document/DocumentChunk 表结构不变，新增表与现有表共存
    - _Requirements: 13.1, 13.2, 13.3_

  - [x] 10.2 编写向量检索往返兼容性属性测试（P2）
    - **Property 17: 向量检索往返兼容性**
    - **Validates: Requirements 13.1**

- [x] 11. Checkpoint — Phase 1 图谱底座就绪
  - 验收标准：
...
- [x] 12. 教学单元数据访问层
  - [x] 12.1 在 `backend/app/repositories/curriculum_repo.py` 中实现教学单元 Repository（Phase 2 部分）
...
- [x] 13. 教学单元生成
  - [x] 13.1 创建 `backend/app/agents/digest/unit_builder.py`，实现局部子图提取与距离计算
  ...
  - [x] 13.2 在 `unit_builder.py` 中实现聚类切割 + 角色分配 + 身份签名
  ...
  - [x] 13.4 在 `unit_builder.py` 中实现 LLM 命名 + 单元 Upsert
...
- [x] 14. CurriculumDeriveJob 工作流 v1（仅 units）
  - [x] 14.1 创建 `backend/app/agents/digest/curriculum_workflow.py`，实现课程派生工作流状态机 v1
...
- [x] 17. 主题树数据访问层
  - [x] 17.1 在 `curriculum_repo.py` 中追加锚点与主题树 Repository
...
- [x] 18. 主题树派生
  - [x] 18.1 创建 `backend/app/agents/digest/theme_tree_builder.py`，实现主题树构建器
  ...
  - [x] 18.2 编写主题树归属唯一属性测试（P0）
    - **Property 3: 主题树归属唯一不变式**
    - **Validates: Requirements 3.5**

  - [x] 18.3 编写锚点优先级稳定性属性测试（P2）
    - **Property 14: 锚点骨架优先级稳定性**
    - **Validates: Requirements 10.2**

- [x] 19. CurriculumDeriveJob 工作流 v2（units + theme tree）
  - [x] 19.1 升级 `curriculum_workflow.py`，启用 derive_theme_tree_node
...
- [x] 22. 先修 DAG 数据访问层
  - [x] 22.1 在 `curriculum_repo.py` 中追加先修 DAG Repository
...
- [x] 23. 先修 DAG 派生
  - [x] 23.1 创建 `backend/app/agents/digest/prereq_dag_builder.py`，实现先修 DAG 构建器
...
- [x] 25. 先修 DAG 服务与 API
  - [x] 25.1 在 `knowledge_graph_service.py` 中追加先修 DAG 服务函数
    - 实现 get_current_prereq_dag、get_current_curriculum_snapshot
    - _Requirements: 12.8, 12.10_ 

  - [x] 25.3 在 `api/knowledge.py` 中追加先修 DAG 路由
...
- [x] 26. Checkpoint — Phase 4 先修 DAG + 完整课程 e2e 就绪
  - 验收标准：
