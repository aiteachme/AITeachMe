...
- [x] 1. 定义枚举与核心数据模型
  - [x] 1.1 在 `backend/app/models/enums.py` 中添加测评相关枚举
  ...
  - [x] 1.2 在 `backend/app/models/assessment.py` 中创建测评数据模型
  ...
  - [x] 1.3 在 `backend/app/models/__init__.py` 中注册新模型
    - 导入所有新模型，确保 SQLModel metadata 能在建表时识别
    - _Requirements: 1.1_


- [x] 2. 实现纯函数核心逻辑与属性测试
  - [x] 2.1 在 `backend/app/agents/profile/review_scheduler.py` 中实现 SM-2 间隔计算
  ...
  - [x] 2.4 在 `backend/app/agents/profile/mastery_updater.py` 中实现掌握度分数计算辅助函数
  ...
  - [x]* 2.5 编写属性测试：UserKnowledgeState 分数与计数不变量（Property 1）
  ...
  - [x]* 2.6 编写属性测试：掌握度加权计算正确性（Property 19）
  ...
  - [x]* 2.12 编写属性测试：ExamPaper 状态机合法迁移（Property 23）
...
- [x] 4. 实现 Repository 层
  - [x] 4.1 创建 `backend/app/repositories/assessment_repo.py`，实现 QuestionTemplate CRUD
  ...
  - [x] 4.2 在 assessment_repo 中添加 ExamPaper 和 ExamPaperItem CRUD
  ...
  - [x] 4.3 在 assessment_repo 中添加 UserAnswerAttempt CRUD
    - `create_answer_attempts`、`list_attempts_by_paper`
    - 强制 exam_paper_item_id + user_id + attempt_no 唯一约束
    - _Requirements: 3.1, 3.2_

  - [x] 4.4 在 assessment_repo 中添加 UserKnowledgeState CRUD
  ...
  - [x] 4.5 在 assessment_repo 中添加 ReviewTask CRUD
  ...
  - [x] 4.6 在 assessment_repo 中添加 Job CRUD
  ...
  - [x] 4.7 在 assessment_repo 中添加组卷所需的跨表读取接口
...
- [x] 5. 实现 Examine_Engine 组件
  - [x] 5.1 在 `backend/app/agents/examine/question_builder.py` 中实现 `question_builder`
  ...
  - [x]* 5.12 编写单元测试：判卷幂等性（Property 20）
    - **Property 20: 判卷幂等性**
    - 重复调用 `grade_paper` 不会修改已判分的作答记录（is_correct、score_obtained、error_cause_label 保持不变）
    - 使用构造型单元测试验证
    - **验证: Requirements 9.1, 9.5, 15.2**

- [x] 6. 检查点 — 确保 Examine_Engine 测试通过
  - 确保所有测试通过，如有问题请询问用户。


- [x] 7. 实现 Profile_Engine 组件
  - [x] 7.1 在 `backend/app/agents/profile/mastery_updater.py` 中实现完整掌握度更新逻辑
  ...
  - [x]* 7.2 编写单元测试：掌握度更新幂等性（Property 21）
  ...
  - [x]* 7.3 编写单元测试：历史试卷归因稳定性（Property 24）
  ...
  - [x] 7.4 在 `backend/app/agents/profile/weakness_analyzer.py` 中实现 `weakness_analyzer`
  ...
  - [x]* 7.8 编写单元测试：单一待处理复习任务（Property 16）
  ...
  - [x]* 7.9 编写单元测试：复习任务去重一致性（Property 22）
    - **Property 22: 复习任务去重一致性**
    - 对相同 updated_state_ids 调用两次 `schedule_reviews` 不会增加 pending ReviewTask 数量
    - 使用构造型单元测试验证
    - **验证: Requirements 12.6, 16.2**

  - [x]* 7.10 编写单元测试：复习模式遗忘优先（Property 13）
    - **Property 13: 复习模式遗忘优先**
    - forgetting_due_at ≤ now 的条目优先于 forgetting_due_at > now 的条目
    - 使用构造型单元测试验证
    - **验证: Requirements 6.5**

- [x] 8. 检查点 — 确保 Profile_Engine 测试通过
  - 确保所有测试通过，如有问题请询问用户。


- [x] 9. 实现 LangGraph 工作流
  - [x] 9.1 在 `backend/app/agents/examine/question_build_workflow.py` 中实现 QuestionBuildWorkflow
  ...
  - [x] 9.2 在 `backend/app/agents/examine/exam_grade_workflow.py` 中实现 ExamGradeWorkflow
...
- [x] 10. 实现 Services 编排层
  - [x] 10.1 创建 `backend/app/services/assessment_service.py`，实现考试生命周期方法
  ...
  - [x] 10.2 在 `assessment_service.py` 中添加掌握度和复习服务方法
  ...
  - [x]* 10.3 编写单元测试：试卷快照不可变性（Property 5）
    - **Property 5: 试卷快照不可变性**
...
- [x] 11. 在 `backend/app/schemas/assessment.py` 中创建请求/响应 Schema
...
- [x] 12. 实现 API 层
  - [x] 12.1 创建 `backend/app/api/assessment.py`，实现所有 REST 端点
  ...
  - [x] 12.2 在 `backend/app/main.py` 中注册 assessment 路由
    - 导入并将 assessment 路由包含到 FastAPI 应用中
    - _Requirements: 19.1_

- [x] 13. 检查点 — 确保所有测试通过，端到端流程可用
  - 确保所有测试通过，如有问题请询问用户。


- [x] 14. 集成联调与数据库初始化
  - [x] 14.1 在数据库初始化中注册测评模型与部分唯一索引
  ...
  - [x] 14.2 为 question_builder 和 answer_grader 编写 LLM 提示词
