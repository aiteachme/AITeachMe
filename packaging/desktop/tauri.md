...
Tauri Windows 只生成 NSIS `.exe` 安装包。NSIS 使用 `lzma` 压缩和 `currentUser` 安装模式，默认安装位置不需要管理员权限，便于本地数据目录留在安装文件夹内。
...
有新版时会提示用户确认；确认后下载清单中声明的已签名 NSIS `.exe` 安装包，验签通过后覆盖安装。Tauri updater 只替换程序和内置资源，不删除安装目录下的 `data` 用户数据目录。
