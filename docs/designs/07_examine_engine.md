...
当前 Examine 的对外资源组已经收口到 `exams`，底层主线是 workflow-backed 的新考试模型。
...
| 2. 组卷 | `paper_assembler.py` / `exams_service` | `subject`、模式、数量、课程版本 | `exam_paper`、`exam_paper_item` |
| 3. 提交答案 | `exams_service.submit_exam_answers` | `exam_paper_id`、答案 | `user_answer_attempt` |
| 4. 判卷 | `exam_grade_workflow.py`、`answer_grader.py` | 试卷与作答 | `user_answer_attempt` |
...
试卷的稳定性来自蓝图层，而不只是题目内容本身。当前正式主线已经收口到“先课程/单元蓝图、后试卷快照”的设计。
...
- 组卷上下文应并入 `exam_paper.selection_context_json`
...
| `paper_assembler.py` | `question_template`、`curriculum_version`、`user_knowledge_state`、`review_task` | `exam_paper`、`exam_paper_item` | 无 |
| `exam_grade_workflow.py` | `exam_paper`、`user_answer_attempt` | `exam_paper.status`、`user_answer_attempt` | 无 |
...
任何继续写 Examine 设计的人，都不应再把 `exam/question/...` 或 `*_job` 当成系统唯一测评模型。

### 9.2 主线应该优先围绕课程版本与模板

Examine 的核心价值在于“可追踪蓝图 + 可快照试卷 + 可回流状态”，这也是它优于旧表链路的地方。

---

## 10. 总结

Examine 当前的正式主线已经明确：围绕 `question_template -> exam_paper -> exam_paper_item -> user_answer_attempt -> user_knowledge_state / review_task` 组织测评闭环。后续开发应继续强化这条主线，不再新增新的 legacy 表、legacy job 表和 `assessment` 命名分支。
