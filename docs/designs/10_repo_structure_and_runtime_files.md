...
- 展示课程：配置 `S3_PUBLIC_BASE_URL` 后读取 OSS index；未配置时不请求 OSS，返回空列表。
- 导入当前环境：`/api/v1/courses/{filename}/import` 在配置 `S3_PUBLIC_BASE_URL` 后可用，由当前连接的后端临时下载 `.atmx` 并导入；成功后出现在左侧学科列表。
