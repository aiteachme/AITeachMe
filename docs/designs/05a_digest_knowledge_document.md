...
- docs lane 应先完成全量材料阅读与主题归并，再进入章节命名与成文；
- 章节名应来源于蓝图决策，不应只从原始标题直接拷贝；
- 运行时应保留可观测参数（时长、token、失败原因），为后续成本与产品化分析提供依据。
...
- 禁止把所有原文直接送进单个 global outline prompt（会导致上下文过载、质量不稳和成本失控）
- 禁止把“整本书成文”作为一次性生成任务 
