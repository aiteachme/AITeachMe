...
如需提升等待态可感知性，可在同一 `POST /knowledge/docs` 响应里附带：

- `build_preview`（可选）：阶段描述、样例节点/卡片、草稿摘录、已处理 chunk 等
- `build_metrics`（可选）：LLM 调用总数、平均时延、按 lane 计数
...
进度条由前端本地生成，不要求后端返回精确进度百分比或精确 ETA。
...
- 后端精确 ETA
- 额外 digest SSE
- 通过额外接口获取 sample cards（如果要展示，走 `POST /knowledge/docs` 可选字段）
