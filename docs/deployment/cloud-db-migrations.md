...
GitHub Actions 的 `deploy.yml` 默认只在对应 Secrets / Repository Variables 齐全时执行真实部署；缺少 Cloudflare deploy hook 或 Sealos kubeconfig / registry 凭证时会自动跳过对应 job。后端镜像也可以通过 `build-backend-images.yml` 手动构建并推送到 GHCR。
