"""Scope retrieval chunk file indexes by course."""
        ...
        sa.text(
            ...
            BEGIN
                IF NOT EXISTS (
                    SELECT 1
                    FROM pg_constraint
                    WHERE conname = 'uq_retrieval_chunk_course_file_id_chunk_index'
                      AND conrelid = 'public.retrieval_chunk'::regclass
                ) THEN
                    ALTER TABLE retrieval_chunk
                    ADD CONSTRAINT uq_retrieval_chunk_course_file_id_chunk_index
                    UNIQUE (course_id, file_id, chunk_index);
                END IF;
            END $$;
            """
        )
...
def downgrade() -> None:
    ...
    op.execute(
        sa.text(
            "ALTER TABLE retrieval_chunk "
            "DROP CONSTRAINT IF EXISTS uq_retrieval_chunk_course_file_id_chunk_index"
        )
    )
    op.execute(
        sa.text(
