...
```bash
curl -X POST http://localhost:8010/api/v1/system/init ^
...
```bash
curl -X POST http://localhost:8010/api/v1/subjects/add ^
  -H "Content-Type: application/json" ^
...
```bash
curl -X POST http://localhost:8010/api/v1/subjects/list ^
  -H "Content-Type: application/json" ^
  -d "{\"page\":1,\"size\":20}"
```
...
```bash
curl -X POST "http://localhost:8010/api/v1/subjects/math/files/upload" ^
...
```bash
curl "http://localhost:8010/api/v1/subjects/math/files"
```
...
```bash
curl -X POST "http://localhost:8010/api/v1/subjects/math/files/delete" ^
  -H "Content-Type: application/json" ^
  -d "{\"file_id\":1}"
```
...
```bash
curl -X POST "http://localhost:8010/api/v1/subjects/math/knowledge/docgen/build" ^
  -H "Content-Type: application/json" ^
...
```bash
curl -X POST "http://localhost:8010/api/v1/subjects/math/knowledge/docgen/get" ^
  -H "Content-Type: application/json" ^
  -d "{}"
```
