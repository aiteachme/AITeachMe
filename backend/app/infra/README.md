...
> 所有 AI 能力集中在此，上层 `workflows/` 通过 `from app.infra.xxx` 调用。
...
| 原则 | 说明 |
|------|------|
| **一行 import，一次调用** | 外部模块使用 infra 只需 `from app.infra.xxx import func` + `await func(...)` |
...
```python
from app.infra.llm import acompletion, acompletion_stream, acompletion_with_tools
...
```python
from app.infra.agent_loop import run_agent_loop, run_agent_loop_stream
...
```python
from app.infra.memory import remember, recall, forget, get_user_profile
...
```python
from app.infra.tools import tool
...
```python
from app.infra.events import emit_event, get_events, count_events, EventType
...
```python
from app.infra.checker import check_answer
...
```python
from app.infra.teaching import run_teaching_function, list_teaching_functions
...
```python
from app.infra.mcp import get_mcp_manager
...
```python
from app.infra.security import check_action_safety, require_confirmation, SecurityLevel
...
| 文件 | 功能 | 使用方式 |
|------|------|----------|
| `prompt_loader.py` | Jinja2 Prompt 模板 | `from app.infra.prompt_loader import populate_prompt` |
| `tracing.py` | 调用追踪 | `from app.infra.tracing import trace_llm_call` |
| `reasoning.py` | 推理策略 | `from app.infra.reasoning import chain_of_thought` |
| `token_budget.py` | Token 预算 | `from app.infra.token_budget import ContextWindowManager` |
| `cache.py` | LLM 缓存 | `from app.infra.cache import cached_completion` |
| `retrievers.py` | 检索管线 | `from app.infra.retrievers import RetrievalPipeline` |
| `reranker.py` | Rerank | `from app.infra.reranker import rerank_chunks` |
| `strategies.py` | 策略编排 | `from app.infra.strategies import StrategyMode` |
