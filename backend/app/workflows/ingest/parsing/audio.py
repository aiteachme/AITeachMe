...
Uses SpeechRecognition library with pydub for format conversion.
Supports: .mp3, .wav, .m4a, .ogg, .flac, .aac, .wma, .opus

Transcription backends (in priority order):
1. Whisper API (if configured via OCR_MODEL containing 'whisper')
2. Google Speech Recognition (free, no API key needed, default)

For best results, configure a Whisper API endpoint.
"""
...
def _ensure_wav(path: Path) -> Path:
    ...
    if AudioSegment is None:
        # Can't convert, try to use the file directly
        # SpeechRecognition can handle WAV, AIFF, FLAC natively
        if path.suffix.lower() in {".flac", ".aiff"}:
            return path
        raise FileParseError(
            path.name,
            reason=f"pydub is required to convert {path.suffix} to WAV. Install pydub and ffmpeg.",
        )
