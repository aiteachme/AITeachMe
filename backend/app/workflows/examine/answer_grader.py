...
def _build_knowledge_context(session: Session, exam_paper_item: ExamPaperItem) -> str:
    ...
    normalized_node_ids = [int(node_id) for node_id in node_ids if isinstance(node_id, int)]
...
async def grade_paper(session: Session, exam_paper_id: int) -> GradeResult:
    ...
    for attempt in attempts:
        ...
        if question_type in {QuestionType.SINGLE_CHOICE.value, QuestionType.FILL_BLANK.value}:
            ...
            try:
                is_correct = await _grade_short_answer_with_llm(
        ...
        if not is_correct:
            ...
            attempt.error_cause_label = await _infer_error_cause_label(
                stem=item.snapshot_stem,
                correct_answer=correct_answer,
                user_answer=user_answer,
            ...
            try:
                await _generate_mistake_analysis(
                    stem=item.snapshot_stem,
                    correct_answer=correct_answer,
                    user_answer=user_answer,
                    knowledge_context=knowledge_context,
