...
def build_profile_pipeline_graph(*, session: Session | None = None) -> StateGraph:
    ...
    workflow = StateGraph(ProfileWorkflowState)
    workflow.add_node("resolve_profile_context", _resolve_profile_context_node(session=session))
    workflow.add_node("update_mastery", _update_mastery_node(session=session))
    workflow.add_node("schedule_reviews", _schedule_reviews_node(session=session))
    workflow.add_node("analyze_weakness", _analyze_weakness_node(session=session))
    workflow.add_node("refresh_subject_profile", _refresh_subject_profile_node(session=session))
    workflow.add_node("refresh_user_profile", _refresh_user_profile_node(session=session))
    workflow.add_node("fail_profile_pipeline", _fail_profile_pipeline_node)
