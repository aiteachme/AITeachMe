...
def route_after_lock(state: KnowledgeDigestState) -> str:
    if state.get("error"):
        return "fail"
    return "prepare"


def route_after_step(state: KnowledgeDigestState) -> str:
    if state.get("error"):
        return "fail"
    return "continue"


def route_after_prepare(state: KnowledgeDigestState) -> str:
    if state.get("error"):
        return "fail"
