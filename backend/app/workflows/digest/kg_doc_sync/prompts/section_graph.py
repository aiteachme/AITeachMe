...
只能使用以上关系类型。节点类型和关系类型不能混用；注意事项、提醒、易错点通常抽成 `misconception` 或 `resource` 节点，并用 `remediates` / `explains` 连接到被说明的内容；练习、自测、纠错任务通常抽成 `skill` 或 `application_case`，并用 `assesses` 连接到被考察的内容。
...
2. 对 `resource`、`misconception`，当被说明或被纠正的上位内容清楚时填写 `parent_entity_name`。
...
3. 如果正文包含概念术语、原理性质、公式模型、方法步骤、解题技能、易错辨析、应用案例或学习资源，要显式抽出对应节点。
