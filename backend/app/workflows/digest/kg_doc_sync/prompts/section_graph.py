...
只能使用以上关系类型。节点类型和关系类型不能混用；注意事项、提醒、易错点通常抽成 `misconception`，并用 `remediates` 连接到被纠正的内容；直观说明、图示、表格或例题可抽成 `application_case` 或 `procedure`，并用 `explains` / `applies_to` 连接到被说明的内容；练习、自测、纠错任务通常抽成 `skill` 或 `application_case`，并用 `assesses` 连接到被考察的内容。`concept` 只用于稳定的知识名词、定义对象或基础事实，不用于“每日 N 道”“练习计划”“测试任务”“应用题建模训练”等学习活动安排。来源材料、阅读链接、普通说明不要单独建节点，应作为证据或摘要保留。

## 关系方向提示
{_ALLOWED_EDGE_DIRECTION_BULLETS}

## 抽取规则
1. 优先抽取可复用的广义学习知识单元，不抽取临时措辞；学校课程、职业技能、生活技能、项目实践都按同一学习功能分类。
2. 名称要短、规范、适合展示；数学符号需要时使用 LaTeX。`name` 只能是学生能复习的知识名词、公式名、方法名、题型名或错误类型，不要带 Markdown 加粗、HTML、整句解释或答案。
3. `local_summary` 只能概括当前片段实际说了什么。
4. 关系端点必须精确匹配本次返回的节点名称。
5. 不要编造当前片段没有依据的节点或关系。
6. 使用 Knowledge Unit 粒度：一个概念/规则、一个公式模型、一个方法步骤、一个解题技能、一个易错辨析或一个应用案例，通常就是合适的单元。
7. 不要为整章、整节、阅读导语、容器标题建节点，除非标题本身就是一个原子概念。
8. 宁可返回 1-3 个强节点，也不要返回一堆弱节点；本片段最多返回 8 个节点、12 条关系，不要把同一小节拆成大量近义主题壳。
9. 严格拒绝空泛教学包装语，例如复习口号、无内容的任务指令、孤立题干句；但如果学习目标、知识框架、总结确实承担组织学习路径的作用，可以抽成 `topic`。
10. `edge_type` 严禁使用节点类型或自造词，例如 `concept`、`procedure`、`support`、`related`；遇到无法归类的弱关系可以不返回。
...
17. 对纯计算题、短练习或短操作案例，节点名应表达共同题型、方法、操作目标或训练目标，例如“基础计算例题”“参数调整方向判断”“步骤排序训练”，不要把孤立算式、答案或一次性任务句直接作为核心节点名，除非它本身就是被讲解的规则或标准。
...
20. 包含“每日/每章/课后/章末/N 道/练习/测试/小测/应用题/综合应用/建模”等训练计划或题型任务意味的节点，优先归为 `skill`；如果只是具体例题或真实场景，归为 `application_case`；不要归为 `concept`。
21. 严禁把“任务 1”“任务 2”“步骤 3（计算）”“检查点”“求解步骤”“本章目标”“单元测试”“每日 N 道题”等容器/编号/流程壳作为节点名；必须压缩成真实知识对象，例如“极限等价替换”“函数连续性判定”“分部积分法”。
22. 如果片段只是在列学习安排、练习数量、检查日期或章末测试说明，且没有可复用的知识/方法/错因名称，可以返回空节点。
...
3. 如果正文包含概念术语、原理性质、公式模型、方法步骤、解题技能、易错辨析或应用案例，要显式抽出对应节点。
4. 如果出现 `定义:`、`公式:`、`例题:`、`练习:`、`备注:` 等标注行，要映射到新的节点类型，不要轻易返回空结果。
...
- 章末单元测试、答案解析、复盘说明只作为技能/易错来源，不能直接抽成“Q01”“任务 1”“检查点”节点；只保留题目背后的考点或错误类型。
- Markdown 样式是辅助信号：二/三级标题、加粗、高亮、callout、表格首列、例题区和练习区通常提示“哪里更可能有可复用知识单元”；但样式本身不是证据，必须从正文含义判断节点与关系。
- 如果正文包含知识图谱表或 Mermaid 图，节点和关系仍要按文本含义重建，关系只能使用本提示词允许的关系类型，不能照抄自造标签。
{% endif %}
