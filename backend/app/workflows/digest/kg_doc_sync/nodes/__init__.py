...
4. ``stitch_relations`` adds conservative no-LLM relation edges between extracted units.
5. ``persist`` merges extracted candidates into knowledge_unit / knowledge_edge tables.
6. ``finalize`` returns the compact report consumed by build runtime.
7. ``fail`` marks the sync run failed when an earlier node cannot continue.
