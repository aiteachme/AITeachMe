...
def build_candidate_name_key(
    ...
    *,
    ...
    return f"{knowledge_unit_type}::{normalize_name(name)}::{normalize_scope_name(scope)}"
...
def build_candidate_stable_id(
    *,
    ...
    return f"{knowledge_unit_type}::{normalize_name(name)}::{normalize_scope_name(scope)}::{local_index}"
...
def bucket_scope(candidate: CandidateLike) -> str:
    ...
    if normalize_knowledge_unit_type(candidate.knowledge_unit_type) in SECONDARY_KNOWLEDGE_UNIT_TYPES:
