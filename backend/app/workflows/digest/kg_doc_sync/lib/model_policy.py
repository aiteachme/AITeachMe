...
_SECTION_GRAPH_TIMEOUT_S = 90
_SECTION_GRAPH_MAX_CONTENT_CHARS = 60000
_SECTION_GRAPH_COURSE_CONTEXT_MAX_CHARS = 1200
...
_POLICIES: dict[KGDocSyncModelStep, KGDocSyncModelPolicy] = {
    KGDocSyncModelStep.SECTION_GRAPH: KGDocSyncModelPolicy(
        ...
        temperature=0.1,
        note="从单个知识文档章节或章节分组抽取高置信候选知识单元和关系；输出预算要容纳最多 10 个节点和 16 条关系的结构化 JSON。",
    ),
...
def kg_doc_sync_section_llm_max_retries() -> int:
    policy = get_kg_doc_sync_model_policy(KGDocSyncModelStep.SECTION_GRAPH)
    return max(1, int(policy.max_retries))
