...
def _visible_visual_texts(elements: list[FigureElement]) -> list[str]:
    values: list[str] = []
    for item in elements[:18]:
        text = item.label or item.text
        if text:
            values.append(text)
        if item.cells:
            values.extend(item.cells[:3])
    return _clean_list(values, limit=18, item_limit=80)


def _has_strong_formula_marker(text: str) -> bool:
    compact = re.sub(r"\s+", "", str(text or ""))
    return bool(
        re.search(
            r"\^|[⁰¹²³⁴⁵⁶⁷⁸⁹]|\\(?:frac|sqrt|sum|int)|[A-Za-z0-9)](?:[+\-*/])(?=[A-Za-z0-9(])",
            compact,
        )
    )


def _is_short_symbolic_token(text: str) -> bool:
    compact = re.sub(r"\s+", "", str(text or ""))
...
def _is_formula_token_relation_diagram(elements: list[FigureElement]) -> bool:
    """Reject algebra-token node graphs that are clearer as inline formulas."""

    items = elements[:18]
    if any(item.kind in {"axis", "curve", "point"} for item in items):
        return False
    if any(item.kind == "shape" and item.shape_type in {"angle", "arc"} for item in items):
        return False

    texts = _visible_visual_texts(items)
    if len(texts) < 3:
        return False
    strong_formula_count = sum(1 for text in texts if _has_strong_formula_marker(text))
    if strong_formula_count < 2:
        return False
    symbolic_count = sum(1 for text in texts if _is_short_symbolic_token(text))
    connection_count = sum(1 for item in items if item.kind in {"line", "vector", "relation"})
    partition_count = sum(
        1
        for item in items
        if item.kind == "shape" and (item.shape_type in {"polygon", "region", "rectangle"} or len(item.points) >= 3)
    )
