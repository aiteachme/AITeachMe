...
_SPRINT_PROFILE = DocGenModeProfile(
    ...
    chapter_format=(
        ...
        "每个考点先给可直接用的规则、公式、步骤或判断口径",
        "规则后接题目、案例、代码/图表任务或小变式，并给解、答案或判定依据",
        "图、表、公式和代码服务关键规则、题目条件或易错边界",
    ),
    course_flow_hints=(
        "考点速览表先定优先级，再按短考点逐个突破",
        "规则抓手后接题/任务，解答后落到记法或检查点",
    ...
    example_density_policy={
        ...
        "policy_text": "紧凑节奏采用高密度练习安排：每章一张考点速览表，完整题/任务约 3 个，短自测、变式或边界辨析约 4-6 个；题目形态由本章材料和章节角色决定。",
    },
    coverage_policy=(
    ...
    mode_writing_rule="紧凑节奏按考点速览、规则抓手、题/任务、解答和错因组织，正文紧凑可检查。",
    prompt_label="紧凑节奏",
    prompt_priority="考点速览、规则抓手、题/任务、解答和错因",
    ...
    prompt_extra_contract="正文采用考点突击讲义形态：考点速览表定优先级；每个考点给规则抓手，再接题/任务、解答和错因。概念章用短例子和反例，方法章给步骤和检查点，训练章按真实题型或任务差异组织。",
)
