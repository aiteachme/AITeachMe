...
_SPRINT_PROFILE = DocGenModeProfile(
    ...
    chapter_format=(
        "开头抓住本章具体对象、方法或任务",
        "按章节角色组织：概念章讲条件和反例，方法章讲步骤和检查点，训练章讲真实任务差异",
        "重要方法落到题目、案例或任务里：条件、步骤、结论、易错边界",
        "例题要写出题目/案例、解析步骤、答案或结论、易错点；自测题必须有答案或解析要点",
        "章末用贴合本章的短练习或任务检查掌握情况",
    ),
    course_flow_hints=(
        "先给本章抓手，再进入概念、方法或任务",
        "方法后接短例题、案例或小变式",
        "结尾回到本章关键条件、任务变化或易错边界",
    ),
    practice_focuses=(
        "由本章内容命名的完整例题或任务",
        "真实条件变化形成的变式",
    ...
    example_density_policy={
        ...
        "policy_text": "快速复习每章保持高密度学习活动：完整例题/案例约 4 个，短练习、自测、变式或边界辨析约 6 个；题目形态由本章材料和章节角色决定。",
    },
    coverage_policy=(
    ...
    mode_writing_rule="快速复习节奏突出可做、可判、可检查；训练密度由本章材料和章节角色决定。",
    prompt_label="快速复习",
    prompt_priority="真实任务分组、完整例题、可执行步骤和易错点",
    ...
    prompt_extra_contract="按本章材料判断章节角色：概念章用短例子和反例讲清条件；方法章给步骤、例题和检查点；训练章按真实题型或任务差异组织。正文里的练习给答案或判定依据。",
)
