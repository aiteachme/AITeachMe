...
def build_presentation_policy(*, digest_mode: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "markdown": {
            ...
            "callout_usage": "每个较长章节自然放 1-2 个短提示块；IMPORTANT 放关键前提/结论，TIP 放快速抓手，WARNING/CAUTION 放易错边界。",
...
- 重点可加粗或少量高亮；每章自然安排 1-2 个短 `> [!IMPORTANT]` / `> [!TIP]` / `> [!WARNING]`，只放关键前提、快速抓手或易错边界；例题和练习用普通正文块。
