...
5. 如果需要新增例题或章末练习收束，要保留题目/任务、推理/解析、答案/结论和易错提醒等学习价值；传统题不适合时改成案例检查、操作步骤检查、边界辨析或迁移任务。
6. 新增标题必须具体说明本章知识对象、方法、题型或操作任务；不要使用固定表头、泛标题或序号占位题型名。
7. 保留已有 `> [!TIP]`、`> [!IMPORTANT]`、`> [!WARNING]`、`> [!EXAMPLE]`、`> [!PRACTICE]` 提示块风格；新增例题优先使用 `> [!EXAMPLE]`，新增章末练习收束优先使用 `> [!PRACTICE]`。
