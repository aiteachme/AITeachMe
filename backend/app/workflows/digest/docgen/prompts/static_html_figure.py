...
如果最合适的是可调参数仿真、动态图、长流程操作或真实图片，不要塞进静态图，visual_kind 写成 interactive_html/chart/generated_image 并说明原因；当前 selected 只保留 static_svg。
...
5. 如果没有任何片段需要示意图，selected 返回空数组。
6. 只输出 JSON 对象。
""".strip()
