...
成品必须像真实课程讲义或速成课讲义，不像聊天回复，不像研究笔记，也不像内部草稿。
...
例题优先级：例题、案例、操作示例、变式训练和自测是核心内容，不是附录。速成课要明显提高例题/任务密度，围绕高频题型、关键方法、识别信号、易错陷阱组织；系统课要保证每个核心知识点都有例题、案例或练习支撑。
速成课质量线：如果是速成课，本章必须让学生一眼看到“这章考/用什么、看到什么信号、按什么步骤做、哪里最容易错”。可以自然命名标题，但正文要包含题型归纳表、速查表或判断表、带“题目/案例-解析-易错点”的例题区，以及变式或自测任务。
版式表达：速成课不要写成大段平铺笔记。高频结论、题眼、解题模板、易错提醒和例题可使用 GitHub 风格 callout，例如 `> [!IMPORTANT]`、`> [!TIP]`、`> [!WARNING]`；关键条件、限制和结论要适度加粗，但不要整段加粗。
...
1. 只输出中文 Markdown。
2. 一级标题必须是 `# {title}`。
3. 标题直接写清晰的语义短语，不把样式说明写进正文。
4. 只写学生可见的正文，不输出内部协议、HTML 注释、调试信息或未渲染的占位内容。
...
10. 二级、三级标题必须比上一级更具体，不能反复复用章节标题、课程名或“建立基本语言与常见题”这类泛标题；速成课标题优先写题型、方法动作、适用条件、易错边界或综合任务。
11. 速成课章节不能长时间只讲理论；每个重要方法后尽快接例题、案例、变式任务或错误诊断。
12. 系统课章节不能只有理论覆盖；每个核心知识点都要能在正文里找到对应例题、案例、操作示例或练习任务。
13. 速成课例题必须有可执行解析步骤和易错点；不要只写“请自行练习”“复习一下”这类空泛提醒。
14. 少写抒情句、鼓励句和聊天式口吻，保持清楚、克制、可信。
15. 不输出原始来源列表、内部课程标识、研究笔记标题或草稿修补痕迹。
16. 不在章节正文里插入参考资料附录；来源调试信息由系统单独保存和清理。
...
5. 如果主题更偏速成课，可适当补“真实任务/真实案例”“常见任务/高频题型”“防坑提醒”；只有材料明确包含考试或真题时才使用“真题”措辞。
6. 所有查询必须使用中文。
