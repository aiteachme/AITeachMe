...
速成课质量线：如果是速成课，本章必须让学生一眼看到“这章考/用什么、看到什么信号、按什么步骤做、哪里最容易错”。可以自然命名标题，但正文要包含速查表或判断表、带“题目/案例-解析-易错点”的例题区，以及变式或自测任务。
...
10. 速成课章节不能长时间只讲理论；每个重要方法后尽快接例题、案例、变式任务或错误诊断。
11. 系统课章节不能只有理论覆盖；每个核心知识点都要能在正文里找到对应例题、案例、操作示例或练习任务。
12. 速成课例题必须有可执行解析步骤和易错点；不要只写“请自行练习”“复习一下”这类空泛提醒。
13. 少写抒情句、鼓励句和聊天式口吻，保持清楚、克制、可信。
14. 不输出原始来源列表、内部课程标识、研究笔记标题或草稿修补痕迹。
15. 不在章节正文里插入参考资料附录；来源调试信息由系统单独保存和清理。
