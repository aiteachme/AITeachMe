...
遇到步骤、例题、易错点和速查信息时，要拆成清楚的信息块，不能让不同学习功能混在同一段里。
...
11. 快速复习章节不能长时间只讲理论；训练型章节每个重要方法后尽快接例题、案例、变式任务或错误诊断，概念型章节也要用短例子、反例或条件辨析支撑理解，题目内容必须由本章知识和材料语义生成。
...
17. 不在章节正文里插入参考资料附录；来源调试信息由系统单独保存和清理。
