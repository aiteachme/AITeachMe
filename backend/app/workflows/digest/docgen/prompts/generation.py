...
本章采用突击讲义式结构：`# 课时/章节标题` 后先给 3-5 行考点/任务导航表，再进入若干个 `## 01 具体知识点名` 小节。
每个知识点小节都必须是本章真实知识对象、方法或任务，不写内部检查词、学习动作、练习配额或模块标签；可见标题编号统一使用两位章内序号，三级标题使用 `01.1` / `01.1.1` 这类层级编号。
标题必须锚定具体知识对象、方法、题型或应用场景；不要只写学习动作、掌握阶段或能力等级。
每个知识点小节内部按“知识点解释 -> 条件/公式/步骤 -> 例题/任务 -> 解析 -> 答案 -> 易错点/检查点”的顺序压缩组织；这些字段用加粗正文标签，不再拆成大标题。
`###` 只在同一个知识点小节下确有两个以上并列子主题时使用；否则改成加粗字段，避免一级标题下面直接跳三级标题。
如果无法确定具体知识点名，宁可合并进相邻知识点小节，不生成假标题。
最后一个二级标题固定为 `## 单元测试`。
...
结构参考大学突击讲义：开头考点/任务表，正文按知识点小节推进，每个知识点穿插题目和解析，最后用单元测试收束。
每章最后一个二级标题必须固定为 `## 单元测试`，里面放本章测试题、任务或案例检查；其它二级标题必须是具体知识点、方法或任务名。
...
蜂考式讲义结构：`# {title}` 后先给一个紧凑考点/任务导航表（考点/任务、重要程度或优先级、常见题型或应用、易错点）；随后只展开若干个知识点小节，例如 `## 绝对值与数轴距离`。每个小节都要自然穿插 **知识点**、**例题/任务**、**解析**、**答案/结论**、**易错点/检查点**，不要把这些字段再拆成二级标题。
标题口径：二级标题来自本章知识对象、公式、方法、真实任务或应用场景；标题要具体、可扫描、有信息增量，并按 `## 01 标题`、`### 01.1 子主题` 的层级编号呈现。
章节边界：执行合同中的“本章边界外主题”属于其它章节；除非本章材料明确用它作一句前后联系，否则不要扩写成独立小节、例题或练习。
版式口径：
{_presentation_contract(digest_mode=normalized_mode)}
内容完整性：每个知识点 `##` 都要形成一个可独立学习的小单元，让学生不看原教材也应能知道“是什么、为什么、什么时候能用、怎么用、怎么检查是否用对”。不要只写几句定义就跳到下一个标题。优先用“概念解释 + 条件边界 + 表格/步骤 + 例题/案例 + 检查点/易错点”的组合压缩信息，让内容比教材更直观、更好复习。
...
4. 正文二级标题必须是 `## 01 具体知识点名` 这类清晰知识点小节，最后一个二级标题固定为 `## 单元测试`；三级标题如出现必须写成 `### 01.1 子主题` 并挂在对应二级标题下，不要制造孤立三级标题，也不要把样式说明写进正文。
...
8. 每个知识点小节至少讲清本小节对象、关键条件或边界、处理路径或解释依据、一个例子/任务/反例，以及一个检查点或易错提醒；如果某项不适合本学科，也要换成等价的案例、操作、证明、设计或诊断标准。
