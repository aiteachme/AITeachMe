...
11. 禁止输出内部或草稿痕迹，例如补全记录、研究材料重组、研究笔记、原始来源列表、内部 subject id 或 ` ```markdown `。
12. 不要在章节正文里插入“参考资料与延伸阅读”；参考资料会统一在文档底部处理。
13. 例题、练习、类比和应用必须来自本章学科语境；除非研究材料明确包含，否则禁止出现无关学科案例，例如缓存命中率、操作系统、营销、物理实验、财务计算等。
14. 如果本章是数学、公式或推导类内容，优先用本章对象构造例题，例如矩阵、向量、方程组、函数、定理条件或符号化小题；材料不足时写“本章自检题”，不要换到别的学科。
15. 突击模式下，如果本章包含公式、计算、判定、证明、算法、系统机制或题型方法，至少写 2 个“例题 + 解析 + 易错点”的短块；例题必须服务本章内容，不要写成泛泛的复习提示。
...
10. 删除或改写任何草稿痕迹，例如补全记录、研究材料重组、研究笔记、原始来源堆砌或 ` ```markdown `。
