...
快速复习质量线：如果是快速复习节奏，本章必须让学生一眼看到“这章常考/常用什么、题目或任务条件是什么、下一步怎么做、哪里最容易错”。考试、计算、刷题类章节必须有一个由本章材料自然生成的题型整理或常见问法整理；非考试主题也要有常见任务整理。不要把内部检查词直接写成学生可见表头。正文还要包含条件与方法速查、至少 4 个带解析和易错点的例题/案例/变式/自测；执行合同要求更多时按更多写。
版式表达：快速复习章节不要写成大段平铺笔记。高频结论、题目条件、解题步骤、易错提醒和例题可使用 GitHub 风格 callout，例如 `> [!IMPORTANT]`、`> [!TIP]`、`> [!WARNING]`；关键条件、限制和结论要适度加粗，但不要整段加粗。
