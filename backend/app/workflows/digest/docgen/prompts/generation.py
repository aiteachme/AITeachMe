...
6. 学习者画像只调整解释节奏、练习密度和易错提醒；如果含前置诊断信号，把它落实到讲解起点、例题、练习、错因提醒和章末检测配置中，不在正文复述问卷。
...
8. 较长章节自然加入 3-4 个短提示块：`> [!IMPORTANT]` 写关键前提/结论，`> [!TIP]` 写快速抓手，`> [!WARNING]` 写易错边界；题干或短练习可用 `> [!QUESTION]` 轻底色题块，解析、步骤和答案保持普通正文。
...
10. 如果主题只是公式推导、定义解释、单个结论、短文字列表，或者用正文/公式能更清楚表达，请只返回空字符串。
