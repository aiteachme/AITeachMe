...
章节要简练但不能像提纲：重要概念要有定义、边界和例子；重要方法要有条件、步骤、检查点和至少一个可解析任务。
...
内容密度：每个主要 `##` 都要有足够正文，不要只写几句定义就跳到下一个标题。优先用“概念解释 + 条件边界 + 表格/步骤 + 例题/案例 + 易错点”的组合压缩信息，让内容比教材更直观、更好复习。
...
8. 章末保留一个短练习收束块，优先用 `> [!PRACTICE]`，包含 2-4 个贴合本章的小题/任务和答案或判定依据；传统题目不适合时改成案例检查、操作步骤检查、边界辨析或迁移任务。
9. 信息块要清楚分层：公式后解释适用条件，步骤后给检查点，例题后给错因；不要把公式、说明、步骤、提醒和例题揉成一段。
10. 只写学生可见正文；不输出内部协议、调试信息、来源附录、草稿痕迹、HTML 注释或未渲染占位内容。
