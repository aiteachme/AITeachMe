...
版式口径：在适合处使用 Markdown 表格、**关键术语加粗**、`> [!TIP]`/`> [!WARNING]`/`> [!IMPORTANT]` 彩色提示块，让定义、条件、易错点和结论有明显层次；每章提示块通常控制在 2-4 个，不要每段都装饰，也不要用花哨符号掩盖内容不足。
...
13. 不要在章节正文里插入“参考资料与延伸阅读”；参考资料会统一在文档底部处理。
...
18. 每章至少要有一个用于强调“条件、结论、易错点或解题入口”的彩色提示块；提示块必须有真实教学信息，不能只写空泛鼓励。
19. 不要使用 emoji 作为标题、列表或段落的结构标记；视觉重点优先通过提示块、表格、加粗和图示体现。
