...
6. 概念判断、选择题、步骤排序、错因辨析、应用迁移、图表读取、推导证明的 options 必须正好 4 项，stem 中不要写 A/B/C/D；答案写正确选项或正确判断。
7. 填空题和短答题的 options 必须是空数组 `[]`。填空题直接在 stem 中留空，在 answer 中写应填内容；短答题在 answer 中写简短结论。步骤排序和错因辨析仍通过四个选项承载。
8. 题目只围绕本章正文里的知识点、方法和易错边界；正文已有练习时，改成同知识点的小变式。
9. basis 写 2-4 个短步骤，优先用 `1. ... 2. ...` 或用分号分隔；每一步说明一个判断依据、计算动作或错因边界。
10. 只输出 JSON 对象；答案是否默认折叠由渲染层负责，你只保证 answer 与 basis 完整、可分行。
