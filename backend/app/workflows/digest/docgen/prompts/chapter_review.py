...
def _compact_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "chapter_end_practice_plan": list(task.get("chapter_end_practice_plan") or [])[:4],
...
3. 快速复习要看章节角色：训练型章节应有题型/任务整理、方法对照、完整例题、变式或自测，不套固定口号或本地模板；概念型章节应有短例子、反例、条件辨析或小任务。缺失时输出 `section_patch`。
4. 每章末尾应有一个短练习收束块：2-4 个贴合本章的小题、案例检查、操作任务、边界辨析或迁移任务，并给出答案、判定依据、解析步骤或结论。缺失、无答案或与本章不贴合时输出 `section_patch`。
5. 自测、辨析或思考题必须有参考答案、判定依据、解析步骤或结论；系统课核心知识点也要有例题、案例、操作示例或练习任务支撑。
6. 检查展示质量：标题层级、加粗/高亮闭合、callout、表格、公式、代码块、Mermaid 是否可渲染。孤立三级标题属于层级过度切分，应要求合并成更具体的 `##` 或改成正文加粗小节。
7. 检查知识图谱相关内容是否只使用 7 类学习节点与 8 类关系；关系方向明显错误时输出 `section_patch`。
8. 如果要求新增或改写小节，不要在 action 里给可直接复制的标题；只说明这个小节要解决什么学习问题，并要求修复模型按本章具体对象、方法、任务差异或场景命名。不要建议目录里看不出内容的泛标题、学习动作标题、内部检查标题或序号占位题型。
9. 复核动作必须可执行，写清 `target_anchor`、`instruction`、`constraints`、`expected_effect`；只做复核判断，不输出修补后的正文。
""".strip()
    messages = [
