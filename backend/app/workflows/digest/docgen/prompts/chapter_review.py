...
5. 每章最后一个二级标题必须固定为 `## 单元测试`，这是唯一固定标题；其它二级标题必须按本章内容自然命名。`## 单元测试` 中紧凑节奏通常 {chapter_end_min_tasks}-{chapter_end_max_tasks} 个短题/任务，系统节奏通常 2-4 个更深的案例检查、操作任务、边界辨析或迁移任务，并给出答案、判定依据、解析步骤或结论。缺失、位置不是最后、无答案或与本章不贴合时输出 `section_patch`。
