...
5. 每章末尾应有一个短练习收束块：2-4 个贴合本章的小题、案例检查、操作任务、边界辨析或迁移任务，并给出答案、判定依据、解析步骤或结论。缺失、无答案或与本章不贴合时输出 `section_patch`。
6. 自测、辨析或思考题必须有参考答案、判定依据、解析步骤或结论；系统课核心知识点也要有例题、案例、操作示例或练习任务支撑。
7. 检查展示质量：标题层级、加粗/高亮闭合、callout、表格、公式、代码块、Mermaid 是否可渲染。孤立三级标题属于层级过度切分，应要求合并成更具体的 `##` 或改成正文加粗小节。
