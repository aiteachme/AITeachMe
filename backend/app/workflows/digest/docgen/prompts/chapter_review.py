...
6. 如果是快速复习节奏，先判断章节角色：训练型章节重点检查例题、案例、变式、自测或实践任务是否足够支撑“会做题/会操作/会判断/会避坑”；概念型章节检查短例子、反例、条件辨析或小任务是否足够支撑理解。密度不足时输出 `section_patch`。
7. 如果是快速复习节奏，只有考试、计算、刷题、方法训练或综合训练章节才强制检查题型或任务整理、方法对照、完整例题、变式题和易错复盘；具体标题、表头和条目应来自章节语义，不应像固定口号或本地模板。
8. 快速复习里的自测、辨析或思考题不能只有问题或提示，必须有参考答案、判定依据、解析步骤或结论；如果缺失，输出 `section_patch` 要求补齐。
9. 如果是系统课，重点检查核心知识点是否都有例题、案例、操作示例或练习任务覆盖；知识点缺少例题覆盖时输出 `section_patch`。
10. 检查展示质量：标题层级、加粗/高亮闭合、callout、表格、公式、代码块、Mermaid 是否可渲染；纯格式问题输出 `surface_patch`，不要升级为整章重写。
11. 检查知识图谱相关内容是否只使用 7 类学习节点与 8 类关系；关系方向明显错误时输出 `section_patch`。
12. 复核动作必须可执行，写清 `target_anchor`、`instruction`、`constraints`、`expected_effect`。
13. 只做复核判断，不输出修补后的正文。
""".strip()
    messages = [
