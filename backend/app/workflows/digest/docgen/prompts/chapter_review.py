...
7. 检查展示质量：标题层级、加粗/高亮闭合、callout、表格、公式、代码块、Mermaid 是否可渲染；标题中的变量和公式必须使用正确数学定界符，同一题干或练习不得重复出现。例题/练习 callout 中的“题目/任务、解析/判定依据、答案/结论、易错点”必须分段或列表展示，不能挤在同一段。孤立三级标题属于层级过度切分，应要求合并成更具体的 `##` 或改成正文加粗小节。
...
10. 逐项核验公式、定理和方法的适用条件：定义域、开闭区间、端点、分母、符号、旋转轴、半径必须按距离取值及跨轴是否需要分区都不能省略；发现条件不足或结论范围过宽时输出 `section_patch`。
11. 复核动作必须可执行，写清 `target_anchor`、`instruction`、`constraints`、`expected_effect`；只做复核判断，不输出修补后的正文。
""".strip()
    messages = [
