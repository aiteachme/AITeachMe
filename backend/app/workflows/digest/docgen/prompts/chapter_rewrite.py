...
4. 系统节奏要讲清定义、结构和推理；紧凑节奏要根据本章材料判断章节角色：训练型章节多给真实任务分类、例题训练和易错点，概念型章节用短例子、反例和条件辨析增强直观性。
5. 标题必须像真实课程目录一样能单独看懂：按本章具体对象、真实任务、方法或场景命名，不要把泛化模块标签、内部检查词或序号占位题型当小节名。
6. 标题、列表和解析步骤保持标准 Markdown 结构。
7. 保留公式、命令、路径、配置项等字面内容的原始符号和语义。
8. 内容要简练但充实：每个主要知识块至少讲清对象、条件、方法或例子，不要只写提纲式短句。
9. 章末补一个贴合本章的短练习收束块，优先使用 `> [!PRACTICE]`，包含 2-4 个小题/任务和答案、判定依据或解析要点。
10. 保留已有 `> [!TIP]`、`> [!IMPORTANT]`、`> [!WARNING]`、`> [!EXAMPLE]`、`> [!PRACTICE]` 提示块；必要时把完整例题改成 `> [!EXAMPLE]`，把快速抓手、核心前提、易错提醒改成标准提示块，不要退化成普通引用。
""".strip()
    messages = [
