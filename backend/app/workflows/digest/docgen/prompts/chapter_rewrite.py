...
9. 章末补一个贴合本章的短练习收束块；单元测试逐题使用独立的 `> [!QUESTION]` 题干块，并把答案、判定依据或解析要点放入紧随其后的 `> [!ANSWER]` 折叠块。
10. 保留已有 `> [!TIP]`、`> [!IMPORTANT]`、`> [!WARNING]`、`> [!QUESTION]` 提示块；把快速抓手、核心前提、易错提醒改成标准提示块，不要退化成普通引用。
""".strip()
    messages = [
