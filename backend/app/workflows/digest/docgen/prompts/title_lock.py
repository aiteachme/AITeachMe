...
优先使用自然、可独立理解的目录标题；如果 confirmed title 像长串枚举，可以收束成更像课程目录的标题，但不要把必要限定词、方法词和场景词都压掉。
...
风格示例只展示“具体、像真实目录、离开上下文也能读懂”的标题形状，不是候选词表，也不能照抄：现金流表、视图生命周期、句法结构、需求拆解、异常处理、实验设计、边界条件判断。
...
9. enhanced_title 不设硬字数，但要适合目录扫描；不要写成长串枚举、冒号副标题或“定义、性质、应用、训练”式堆叠。
10. 如果 enhanced_title 只表达学习阶段或能力目标，而没有写出本章具体内容对象，就不是合格标题。
11. 不要输出任何教学大纲、检索词、媒体请求或其他字段。
""".strip()
    messages = [
