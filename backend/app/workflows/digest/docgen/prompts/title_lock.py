...
标题不是标签，不要只写抽象分类；自然表达本章的知识对象、方法任务、题型场景或能力目标。
避免空泛比喻、拟人或文学化表达。
标题自然即可，避免口号化、过度对仗或统一句式。
标题本身不要包含编号或样式说明。
可参考优质突击课讲义的清晰度和颗粒度，但不要照抄任何资料标题，也不要从固定候选词里拼标题。
成熟考前复习课的标题通常先暴露题型、未定式、方法选择、适用前提、错误边界或综合训练，不会反复使用同一个抽象标题；同一章内相邻标题必须有明显的信息增量。
""".strip()
    prompt = f"""
请为下面这一章锁定最终发布标题。
...
8. 不要为了押韵、对仗或统一句式批量改写标题；相邻章节标题必须各自自然。
9. 应试/突击模式下，标题要更直接说明本章的题型、方法、判断动作或易错任务；系统学习模式下，标题要更直接说明概念对象、结构关系或应用边界。
10. 不要把多个章节或小节都命名成同一类抽象句式；如果标题之间只差少量形容词，必须改成更具体的题型、方法或条件差异。
11. 不要从固定标签里拼标题；必须基于本章上下文做语义命名。
12. 不要输出任何教学大纲、检索词、媒体请求或其他字段。
""".strip()
    messages = [
