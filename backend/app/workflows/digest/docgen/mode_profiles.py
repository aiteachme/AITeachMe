...
_SPRINT_PROFILE = DocGenModeProfile(
    ...
    course_flow_hints=(
        ...
        "讲完一组由模型判定的真实任务后，用例题解析、变式训练和易错辨析收束；不适合训练的章节用具体结论、条件边界或短例子收束",
    ),
    practice_focuses=(
        "由本章内容命名的完整例题或任务",
        "真实条件变化形成的变式",
    ...
    prompt_extra_contract="快速复习不是把每章套成同一种题型表。请先由模型根据材料判断本章角色：训练型章节要形成由真实任务差异命名的学习路径，用完整例题和变式题教会做法；概念型或过渡型章节用少量短例子、反例和边界提醒讲清条件。标题、表头和题目必须由本章内容自然命名，不要把泛化模块标题当作默认收尾。",
)
