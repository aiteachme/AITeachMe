...
_SPRINT_PROFILE = DocGenModeProfile(
    ...
    chapter_format=(
        ...
        "每章末尾保留短练习收束块，具体题目、任务形态和答案依据由模型根据本章内容生成，不套固定模板",
    ),
    course_flow_hints=(
    ...
    coverage_policy=(
        ...
        "例题、测验和综合训练一般集中在自然适合的 1-2 个章节或章末；其他章节也要有短练习收束，但题目形态应按本章内容自然生成，不要为凑结构硬写自测。",
    ...
    prompt_extra_contract="快速复习不是把每章套成同一种题型表。请先由模型根据材料判断本章角色：训练型章节要形成由真实任务差异命名的学习路径，用完整例题和变式题教会做法；概念型或过渡型章节用少量短例子、反例和边界提醒讲清条件。每章末尾要有短练习收束块，但题目形态必须由本章内容自然生成。标题、表头和题目必须由本章内容自然命名，不要把泛化模块标题当作默认收尾。",
)
