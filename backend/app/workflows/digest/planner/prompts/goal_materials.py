...
- 输出一段自然语言正文。
- 控制在 180-420 字。
- 说清楚：用户真正想学什么、时间约束和产出要求是什么、方案应该按什么主线拆。
- 用户已经列出的章节、模块、知识点或范围是拆分主线的优先依据；当输入呈现 A/B/C 列表时，直接把 A/B/C 作为章节边界。
- 用户给出 A/B/C 这类章节列表时，planning_note 写成 A/B/C 的逐项学习路径，列表项就是完整章节边界。
- 用户同时给出学习天数和 A/B/C 列表时，天数是这些列表项的进度预算；每个列表项按自身知识内容展开。
- 练习、检测、纠错、巩固是对应内容模块内的学习活动，写进相应模块的学习安排。
