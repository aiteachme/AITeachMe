...
5. 用户以“按 A、B、C 划分章节/模块/单元”给出列表时，chapters 与 A/B/C 逐项对应：第 i 个 chapter 负责第 i 个列表项，数组长度等于列表项数量，标题沿用或自然概括该列表项。
6. 练习、检测、错因复盘进入对应模块的 key_points；综合卷、跨模块训练、考前模拟作为独立 chapter 的来源是用户把它列入章节列表，或在目标中提出这类训练需求。
7. plan 的先后顺序以用户给出的列表项为完整路径，练习、回顾和检测以各项内嵌任务表述。
