...
- 示例只展示“短、具体、像真实目录”的标题形状，不是候选词表，也不能照抄：知识对象型如“现金流表”“视图生命周期”“句法结构”；方法任务型如“需求拆解”“异常处理”“实验设计”；问题场景型如“边界条件判断”“证据链梳理”“方案取舍题”。
- 如果用户材料与示例领域不同，必须回到用户目标、资料画像和章节语义，生成同等清晰但贴合本课的新标题。
- key_points 才写学习动作、练习方式、易错边界和题型处理；不要把这些动作压进 title。
- 对完整课程，优先按该领域真实教材、课程或任务边界拆分；速成模式只影响取舍和 key_points 密度，不把标题改成抽象能力标签。
- 对窄范围专题，title 仍然是该专题内部的自然目录名，围绕它真正要讲清的对象、步骤、场景或题型命名；不要扩展成整门课，也不要套“核心概念”“方法总结”“学习路径”。
...
- 示例只用于说明“不要扩展范围”：如果用户要一个具体公式、算法、实验方法、业务报表或章节主题，chapters 应拆成它的适用场景、前提边界、操作步骤、典型任务、易错诊断和综合练习；不能自动扩成整门课。
- 如果用户明确指定 N 个章节，必须保持 N 章并全部围绕该范围展开；可以按定义/边界、方法/步骤、题型/任务、应用/综合、错因/检查等真实教学视角拆开，具体标题仍由本领域语义决定。
