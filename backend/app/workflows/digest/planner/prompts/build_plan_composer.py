...
4. chapters：输出完整章节列表；title 用清楚直观的课程目录名，通常 6-18 字，聚焦一个核心知识对象、方法任务、题型技能或应用场景；key_points 2-4 条承接细节、例题、练习或测验。
...
7. 练习、检测、纠错、巩固进入对应内容模块的 key_points；用户列出的额外学习活动也按其所在列表位置处理。
8. plan 的先后顺序以用户给出的列表项为完整路径，各模块内部再安排练习、纠错和小测；用户同时给出学习天数和 A/B/C 列表时，天数是 A/B/C 的进度预算，末尾时间仍属于最后一个列表项。
