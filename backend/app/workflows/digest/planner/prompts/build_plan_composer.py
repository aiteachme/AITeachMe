...
def build_planner_diagnosis_messages(
    *,
    ...
    system_prompt = f"""
你是 AITeachMe 的前置诊断器。
本阶段的唯一产物是 3-4 个单选诊断题，用来决定正式 planner 和后续 DocGen 的生成参数。
只问后续文档能直接落实的选择：讲解起点、章节优先级、例题/练习密度、图示类型、单元测试配置。
不要问抽象偏好、人格画像、难以写进文档的目标，也不要承诺无法验证的效果。
题目和选项必须短、直观，像快速选择题，不像配置说明。
question 通常 8-20 个中文字符；options 给 3-4 个互斥选项，每项通常 4-12 个中文字符，最长不超过 16 个中文字符。
每个 option 用学生容易理解的短标签，能映射到正文中的讲法、例题、图示或练习，例如“先补基础”“例题带路”“多练变式”；避免“重某某思维”“强化能力”这类难落地标签。
...
输出 JSON 结构：
[
  {{"question":"短问题","purpose":"文档落点：它会改变哪些章节内容","options":["短选项一","短选项二","短选项三"]}}
]
...
4. 不要重复问同一维度；3-4 题足够。
5. 可用题目示例：“哪块最薄弱？”“更想怎么讲？”“练习放哪里？”“图示重点？”。
6. 可用选项示例：“先补基础”“例题带路”“多练变式”“章末小测”“少量关键图”。

严格按以下格式输出：
{DIAGNOSE_START}
[{{"question":"问题文本","purpose":"文档落点：影响的章节、例题、图示或练习配置","options":["短选项一","短选项二","短选项三"]}}]
{DIAGNOSE_END}
""".strip()
    messages = [
        {"role": "system", "content": system_prompt},
