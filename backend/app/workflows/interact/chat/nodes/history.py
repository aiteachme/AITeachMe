...
from app.repositories import profile_repo, course_repo
...
def build_load_history_state_node(*, context: WorkflowContext, session: Session | None = None):
    ...
    def load_history_state(state: InteractWorkflowState) -> InteractWorkflowState:
        with _node_session(session) as db_session:
            ...
            course_context = _load_course_context(
                db_session,
                course_id=course_id,
                user_id=state["user_id"],
            )
...
def _load_course_context(
    ...
    *,
    ...
    record = course_repo.get_course_by_id(session, course_id, owner_user_id=user_id)
    if record is None:
        record = course_repo.get_course_by_id(session, course_id)

    try:
        profile = build_course_profile_summary(session, course_id=course_id, user_id=user_id)
    except Exception:
        profile = None

    return CourseContextSummary(
        course_id=course_id,
        course_name=(record.name if record and record.name else "未命名课程"),
        ...
        course_intro=record.course_intro_text if record else "",
