    ...
    sections = [
        ...
        SettingSection(
            ...
            entries=[
                ...
                lse("models.primary", "Primary 模型", settings.models.primary, base_settings.models.primary, "主要生成、对话、批改任务。"),
                lse("models.reason", "Reason 模型", settings.models.reason, base_settings.models.reason, "深度推理与规划；空值时回退到 Primary。"),
                lse("models.light", "Light 模型", settings.models.light, base_settings.models.light, "分类、摘要和批量轻任务；空值时回退到 Primary。"),
                lse("models.extract", "Extract 模型", settings.models.extract, base_settings.models.extract, "知识抽取专用；空值时回退到 Light/Primary。"),
                lse("models.embedding", "Embedding 模型", settings.models.embedding, base_settings.models.embedding),
                _runtime_entry("models.embedding_dim", "Embedding 维度", settings.embedding_dim),
                lse("models.ocr", "Vision OCR 模型", settings.models.ocr, base_settings.models.ocr, "空值时使用 Primary 模型；密钥和服务地址复用 LLM 接入配置。"),
        ...
        SettingSection(
            ...
            entries=[
                lse("rag.top_k", "RAG Top-K", settings.rag.top_k, base_settings.rag.top_k),
                ...
                lse("search.retriever_profile", "检索 Profile", settings.search.retriever_profile, base_settings.search.retriever_profile),
                lse("search.retrievers", "显式 Retrievers", settings.search.retrievers, base_settings.search.retrievers),
                lse("search.max_results_per_query", "单 query 最大结果数", settings.search.max_results_per_query, base_settings.search.max_results_per_query),
                lse("search.scrape_timeout_s", "网页抓取超时", settings.search.scrape_timeout_s, base_settings.search.scrape_timeout_s),
                lse("search.provider_timeout_s", "Provider 超时", settings.search.provider_timeout_s, base_settings.search.provider_timeout_s),
                lse("search.total_timeout_s", "总检索预算", settings.search.total_timeout_s, base_settings.search.total_timeout_s),
                lse("search.read_timeout_s", "Reader 超时", settings.search.read_timeout_s, base_settings.search.read_timeout_s),
                lse("search.parallel_retrievers", "并发检索", settings.search.parallel_retrievers, base_settings.search.parallel_retrievers),
                lse("search.max_parallel_retrievers", "最大并发 Provider", settings.search.max_parallel_retrievers, base_settings.search.max_parallel_retrievers),
                ...
                _runtime_entry("search.retriever_profiles", "检索 Profile 覆盖表", settings.search.retriever_profiles, "当前仅从代码默认值与可选项目 override 读取，作为高级配置保留。"),
            ],
        ),
        SettingSection(
            id="storage",
        ...
        SettingSection(
            ...
            entries=[
                lse("observability.tracing_enabled", "Tracing 总开关", settings.observability.tracing_enabled, base_settings.observability.tracing_enabled),
                ...
                lse("embedding.batch_size", "Embedding 批大小", settings.embedding.batch_size, base_settings.embedding.batch_size),
                lse("embedding.batch_delay_s", "Embedding 批延迟", settings.embedding.batch_delay_s, base_settings.embedding.batch_delay_s),
            ],
        ),
    ]
