...
class SQLiteVecVectorStore(BasePydanticVectorStore):
    ...
    def add(self, nodes: Sequence[BaseNode], **kwargs: Any) -> list[str]:
        ...
        if not normalized_subject or not nodes:
            return []
        ...
        for node in nodes:
            ...
            if chunk_id is None:
                continue
            node_subject = str(node.metadata.get("subject") or normalized_subject).strip()
            if node_subject != normalized_subject:
                raise ValueError(
                    f"Node {node.node_id} belongs to subject {node_subject!r}, "
                    f"expected {normalized_subject!r}."
                )
    ...
    def delete_nodes(
        ...
        **delete_kwargs: Any,
        ...
        if not normalized_subject or not chunk_ids:
            return
        ...
        with get_engine().connect() as connection:
            ...
            for table_name in _list_vector_tables(connection):
                ...
                count = connection.execute(
                    ...
                    {"subject": normalized_subject, **params},
                ).scalar_one()
