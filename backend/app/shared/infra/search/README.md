...
- `readers/`
  负责把 URL 变成结构化可消费内容，例如 HTML / PDF / DOCX / PPTX / Markdown / TXT 抽取。
- `knowledge.py`
...
旧 `app.shared.infra.search.scraper` 目录现在只保留兼容 shim。
`app.shared.infra.retrievers` 与 `app.shared.infra.reranker` 目前也只保留兼容 shim，不再作为新的 canonical 入口。
