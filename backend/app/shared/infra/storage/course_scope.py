"""Course-scoped storage namespace helpers."""
...
@dataclass(frozen=True)
class CourseStorageScope:
    """Canonical persisted storage namespace for one user-owned course."""
    ...
    @property
    def namespace(self) -> str:
        return f"users/{self.user_segment}/courses/{self.course_id}"
...
def build_course_storage_scope(*, user_id: str, course_id: str) -> CourseStorageScope:
    """Create the canonical storage scope for one user-owned course."""

    return CourseStorageScope(
...
def resolve_course_storage_scope(course_id: str, *, session: Session | None = None) -> CourseStorageScope:
    """Resolve a course id into its persisted storage scope."""

    normalized_course = validate_course_id(course_id)
    if session is not None:
        record = session.exec(select(Course).where(Course.id == normalized_course)).first()
        if record is None:
            raise CourseRegistryNotFoundError(normalized_course)
        return build_course_storage_scope(
