...
对外使用::
    ...
    # 记录事件
    await emit_event("exam_completed", user_id="u1", course_id="math",
...
class _EventStore:
    ...
    def _ensure_table(self) -> None:
        ...
        try:
            ...
            """)
            try:
                conn.execute("ALTER TABLE teaching_events ADD COLUMN course_id TEXT DEFAULT ''")
            except Exception:
                pass
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_events_course
                    ON teaching_events(user_id, course_id, created_at)
            """)
            conn.commit()
    ...
    async def save(self, event: TeachingEvent) -> None:
        ...
        try:
            conn.execute(
                """INSERT INTO teaching_events (id, event_type, user_id, course_id, data, created_at)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (event.event_id, event.event_type, event.user_id,
                 event.course_id, json.dumps(event.data, ensure_ascii=False),
    ...
    async def query(
        ...
        *,
        ...
        conn = self._get_conn()
        try:
            sql = "SELECT id, event_type, user_id, course_id, data, created_at FROM teaching_events WHERE user_id = ?"
            params: list = [user_id]
...
async def get_events(
    *,
    ...
    return await _get_store().query(
        ...
        course_id=course_id, days=days, limit=limit,
    )
