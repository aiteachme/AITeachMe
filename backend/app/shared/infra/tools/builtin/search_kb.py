...
async def search_kb_tool(query: str, course_id: str, top_k: int = 5) -> str:
    """搜索知识库。"""

    from app.shared.infra.search import get_knowledge_search_notice, search_knowledge

    search_notice = await get_knowledge_search_notice(course_id)
    if search_notice is not None:
        return search_notice

    chunks = await search_knowledge(query, course_id, top_k=top_k)
    if not chunks:
        return "知识库中未找到相关内容。"
