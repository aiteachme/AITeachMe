...
更新 Knowledge.pipeline_stage �?embedded�?
"""
...
async def embed_chunks(chunks: list[ChunkData]) -> list[list[float]]:
    ...
    将每�?chunk �?title + content 拼接作为嵌入输入文本�?
...
def save_chunks_and_embeddings(
    ...
    写入 Chunk 表记录，然后批量插入 embedding �?chunk_embeddings 虚拟表�?

    Args:
        session: 数据库会话�?
        knowledge_id: 关联�?Knowledge ID�?
        chunks: 分块数据列表�?
