...
def _workflow_logger(state: CurriculumDeriveState) -> structlog.stdlib.BoundLogger:
    """为当前课程派生流程绑定统一日志上下文。"""
...
async def derive_units_node(state: CurriculumDeriveState) -> CurriculumDeriveState:
    ...
    with managed_session() as session:
        try:
            ...
            workflow_logger.error("derive_units_failed", error=str(exc), exc_info=True)
...
async def derive_theme_tree_node(state: CurriculumDeriveState) -> CurriculumDeriveState:
    ...
    with managed_session() as session:
        try:
            ...
            workflow_logger.error("derive_theme_tree_failed", error=str(exc), exc_info=True)
...
async def derive_prereq_dag_node(state: CurriculumDeriveState) -> CurriculumDeriveState:
    ...
    with managed_session() as session:
        try:
            ...
            workflow_logger.error("derive_prereq_dag_failed", error=str(exc), exc_info=True)
...
async def finalize_curriculum_node(state: CurriculumDeriveState) -> CurriculumDeriveState:
    ...
    with managed_session() as session:
        try:
            ...
            workflow_logger.info("curriculum_activated", activated=activated)

            # 2. publish tree version（draft → published）
            if tree_version_id is not None:
                publish_theme_tree_version(session, version_id=tree_version_id)
                workflow_logger.info("tree_version_published", version_id=tree_version_id)

            # 3. publish dag version（draft → published）
            if dag_version_id is not None:
                publish_prereq_dag_version(session, version_id=dag_version_id)
                workflow_logger.info("dag_version_published", version_id=dag_version_id)
            ...
            if tree_version_id is not None or dag_version_id is not None:
                ...
                workflow_logger.info("curriculum_snapshot_published", snapshot_id=snapshot_id)
            ...
            workflow_logger.error("curriculum_finalize_failed", error=str(exc), exc_info=True)
...
async def fail_curriculum_node(state: CurriculumDeriveState) -> CurriculumDeriveState:
    ...
    with managed_session() as session:
        try:
            ...
            workflow_logger.error("curriculum_workflow_failed", error=error_msg)
            return state
        except Exception as exc:
            workflow_logger.error("curriculum_fail_node_error", error=str(exc), exc_info=True)
            return state
