"""
系统提示词构�?�?Interact 引擎上下文组�?

注入最�?N 轮对话摘要（内存中，无需额外表）�?
...
_SYSTEM_PROMPT_BASE = """你是 AITeachMe �?AI 学习助手，专注于为学生提供个性化的学科辅导�?
...
def build_system_prompt(
    ...
    *,
    ...
    Returns:
        包含 system 消息�?messages 列表（不含用户当前问题）�?
    """
...
def _build_retrieval_context(results: list[RetrievalResult]) -> str:
    ...
    for i, r in enumerate(results, 1):
        relevance = "⚠️低相�? if r.low_relevance else "✓相�?
        lines.append(
...
def _build_selected_context(
    ...
    header = "\n## 用户选中的重点内容（高优先级�?
    if source_chunk_id:
        header += f"（来�?chunk_id: {source_chunk_id}�?
    return f"{header}\n{selected_context}"
...
def _build_weak_points_context(weak_points: list) -> str:
    """格式化薄弱知识点列表�?""
    lines = ["\n## 学生薄弱知识点（请重点关注）"]
    for wp in weak_points[:10]:  # 最多展�?10 �?
        mastery_pct = f"{wp.mastery * 100:.0f}%" if wp.mastery is not None else "未测�?
