...
@router.post(
    "/mock/full-markdown",
    response_model=ApiResponse[DocGenContentResponse],
    summary="获取学科知识总 Markdown（Mock 超长文本）",
    responses=build_error_responses([400, 404, 500]),
)
async def mock_full_markdown(
    subject: str = Path(...),
    session: Session = Depends(get_db),
) -> ApiResponse[DocGenContentResponse]:
    normalized = normalize_subject_slug(subject)
    get_subject_record(session, normalized)
    mock_markdown = _build_mock_middle_east_markdown(normalized)
    return ok_response(DocGenContentResponse(markdown=mock_markdown))
