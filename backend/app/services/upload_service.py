...
流程：校�?�?临时落盘 �?创建 RawFile �?移动到最终位�?�?解析 �?创建 Knowledge �?触发 Digest�?
...
async def handle_upload(
    ...
    """处理文件上传：校�?�?临时落盘 �?创建记录 �?移动到最终位置�?

    Returns:
        创建�?RawFile 记录（含 task_id = raw_file.id）�?
    """
...
async def process_and_parse(session: Session, raw_file_id: int) -> Knowledge | None:
    ...
    流程：更�?parse_status=parsing �?调用解析�?�?保存 Markdown �?更新 parsed �?创建 Knowledge�?
    失败时更�?parse_status=parse_failed�?

    Returns:
        创建�?Knowledge 记录，解析失败时返回 None�?
    """
    ...
    # 保存 Markdown �?data/markdown/<prefix>/<id>.md
    data_dir = _get_data_dir()
    ...
    knowledge = Knowledge(
        ...
        title=title,
        markdown_content="",  # Digest 引擎�?store_knowledge 节点会填�?
...
def compute_pipeline_status(
    ...
    """根据 parse_status �?pipeline_stage 计算聚合状态�?
    ...
    if knowledge is None:
        return {"stage": "parse", "progress": 30, "message": "解析完成，准备索�?, "error": None}
    ...
    stage, progress, message = stage_map.get(
        knowledge.pipeline_stage, ("digest", 40, "处理�?)
    )
