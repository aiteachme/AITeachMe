...
class RetrievalDebugRequest(BaseModel):
    """Debug query for subject knowledge retrieval."""

    query: str = Field(min_length=1, description="Query text used for local subject retrieval.")
...
class RetrievalDebugResponse(BaseModel):
    """Debug payload for one subject knowledge retrieval call."""
...
class KnowledgeGraphBuildMetricsResponse(BaseModel):
    ...
    stitched_edge_count: int = Field(default=0, description="No-LLM relation-stitching edges added before graph persistence.")
    section_local_stitch_edge_count: int = Field(default=0, description="Edges stitched inside the same section to reduce isolated units.")
    mention_stitch_edge_count: int = Field(default=0, description="Edges stitched from explicit cross-unit mentions in extracted text.")
    graph_isolated_unit_count: int = Field(default=0, description="Extracted units with zero incident edges after relation stitching.")
    graph_component_count: int = Field(default=0, description="Undirected connected component count after relation stitching.")
    graph_largest_component_unit_count: int = Field(default=0, description="Largest connected component size after relation stitching.")
    graph_active_unit_count: int = Field(default=0, description="Extracted active-unit count used for graph health metrics.")
    graph_active_edge_count: int = Field(default=0, description="Extracted active-edge count used for graph health metrics.")
    graph_avg_degree: float = Field(default=0.0, description="Average undirected degree after relation stitching.")
    graph_isolated_unit_pct: float = Field(default=0.0, description="Percentage of extracted units with no incident edge after relation stitching.")
