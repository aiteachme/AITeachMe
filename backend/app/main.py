...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    from app.workflows.support.auth import run_auth_housekeeping_loop, run_guest_merge_cleanup_loop
    ...
    app.state.background_task_registry.spawn(
        ...
        kind="auth.merge.cleanup",
        name="auth.merge.cleanup",
        dedupe_key="auth.merge.cleanup",
    )
    app.state.background_task_registry.spawn(
        run_auth_housekeeping_loop(),
        kind="auth.housekeeping",
        name="auth.housekeeping",
        dedupe_key="auth.housekeeping",
    )
    app.state.background_task_registry.spawn(
