...
class QuestionTypeRegistry(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("scope", "course_id", "type_key", name="uq_question_type_scope_course_key"),
    )
    ...
    course_id: str = Field(default="", index=True)
...
class ExamPaper(SQLModel, table=True):
    """Generated exam paper for one course and one user."""
