...
flowchart TD
    ...
    class __start__ startCls
    class __end__ endCls
    linkStyle 1,3,4,6 stroke:#f43f5e,stroke-dasharray:5
```
...
| 确认章节生成计划 | 🔀 条件路由 | `fail` -> END / `continue` -> 构建文档知识骨架 / `Send xN` -> 生成章节草稿 |
| 构建文档知识骨架 | ⚙ 处理节点 | `fail` -> END |
| 生成章节草稿 | ⚙ 处理节点 | → 增强章节内容 |
