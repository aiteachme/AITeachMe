...
def test_collect_planner_concept_briefing_merges_local_and_web_evidence(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.workflows.digest.planner.internal.grounding.get_settings",
        lambda: SimpleNamespace(web_search_retriever="duckduckgo", planner_allow_external_search=True),
    )
    monkeypatch.setattr(
        "app.workflows.digest.planner.internal.grounding.get_retriever",
        lambda name, **kwargs: FakeRetriever(name),
    )
    monkeypatch.setattr(
        "app.workflows.digest.planner.internal.grounding.read_urls",
