...
def test_legacy_knowledge_graph_types_are_normalized_for_compatibility():
    ...
    assert normalize_generated_knowledge_unit_type("resource", name="标准化数据提醒") == "concept"
    assert normalize_generated_knowledge_unit_type("remark", name="应用图示") == "concept"
    assert normalize_generated_knowledge_unit_type("concept", name="每日 8 道方程求解与 2 道应用题建模") == "concept"
    assert normalize_generated_knowledge_unit_type("skill", name="章末小测：函数图像性质辨析") == "skill"
