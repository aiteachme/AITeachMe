...
from app.workflows.digest.docgen.builds import trigger_docgen_build
...
def test_trigger_docgen_build_force_full_rebuild_clears_chunk_vector_metadata(
    ...
    with patch(
        "app.workflows.digest.docgen.builds.inspect_subject_build_precheck",
        return_value=conflict,
    ), patch(
        "app.workflows.digest.docgen.builds.resolve_subject_build_vector_status",
        return_value=_vector_status(),
    ), patch(
        "app.workflows.digest.docgen.builds.acquire_knowledge_build_lock",
        return_value=True,
    ), patch(
        "app.workflows.digest.docgen.builds.clear_docgen_staging",
    ), patch(
        "app.workflows.digest.docgen.builds.update_knowledge_build_status",
    ):
...
def test_trigger_docgen_build_calls_direct_clear_chunk_vector_metadata(session: Session) -> None:
    ...
    with patch(
        ...
        "app.workflows.digest.docgen.builds.clear_chunk_vector_metadata",
    ) as clear_mock, patch(
        "app.workflows.digest.docgen.builds.acquire_knowledge_build_lock",
        return_value=True,
    ), patch(
