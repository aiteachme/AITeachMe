...
export function SubjectImportModal({ onClose, onImported }: SubjectImportModalProps) {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      toast({
        title: "导入成功",
        description: `${result.subject_name} 已加入学科列表。`,
        variant: "success",
      });
      onImported?.(result);
      onClose();
      ...
      <div className="relative z-10 flex max-h-[calc(100dvh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-[0_18px_54px_rgba(15,23,42,0.16)] dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-800 sm:px-6 sm:py-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">从 .atmx 包恢复完整学科、知识文档和学习记录</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6">
          ...
          <label className="mt-4 block">
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">导入后的学科名称（可选）</span>
            <input
              type="text"
              value={customName}
