    ...
    <CourseOperationModal
      ...
      sidebar={
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/70 bg-white/70 p-4 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/35">
            <div className="text-xs font-medium text-slate-500 dark:text-slate-400">预计写入</div>
            <div className="mt-2 font-mono text-2xl font-semibold tracking-tight text-slate-950 dark:text-slate-50">
              {previewQuery.isLoading ? "--" : selectedDynamicCount + coreCount}
            </div>
            <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">条课程记录</div>
          </div>
          <div className="space-y-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
            <p>始终包含课程信息、知识图谱节点与关系。</p>
            <p>原始上传文件、向量索引、临时构建状态和运行时锁不会导出。</p>
          </div>
        </div>
      }
      footer={
    ...
    >
      {previewQuery.isLoading ? (
        <div className="space-y-4">
          ...
          {[0, 1, 2, 3].map((item) => (
            <div key={item} className="flex items-center gap-3 rounded-2xl border border-slate-100 px-4 py-4 dark:border-slate-800">
        ...
        <div className="space-y-5">
          <section className="border-b border-slate-100 pb-5 dark:border-slate-800">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h3 className="text-base font-semibold tracking-tight text-slate-950 dark:text-slate-50">打包内容</h3>
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/35">
          ...
          <section className="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200 dark:divide-slate-800 dark:border-slate-800">
            {optionRows.map((row) => {
              const Icon = row.icon;
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                    <Icon className="h-4 w-4" />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="flex items-start justify-between gap-3">
                      <span className="text-sm font-semibold text-slate-950 dark:text-slate-50">{row.title}</span>
                      <span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                        {row.count}
                      </span>
                    </span>
      ...
      {exportMutation.isError ? (
        <div className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300">
