...
function TabFallback({ message }: { message: string }) {
  return (
    <div className="flex h-full min-h-[360px] items-center justify-center bg-white text-sm text-slate-500">
        ...
        <div className="flex min-w-0 items-center gap-2">
          {onClose && (
            <>
              <button
                type="button"
                onClick={onClose}
                className="flex items-center justify-center gap-1 rounded-md px-2 py-1.5 text-[13px] font-medium text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800"
              >
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-slate-900">知识图谱</p>
              <p className="hidden truncate text-xs text-slate-500 lg:block">查看知识点、关系和文档来源</p>
            </div>
          </div>
        </div>
