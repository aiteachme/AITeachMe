...
import { EXAM_ANSWER_TEXT_CLASS, EXAM_QUESTION_TEXT_CLASS, ExamMarkdown } from "./ExamMarkdown";
import {
  buildKnowledgeLabel,
...
function AnalysisBlock({ title, content }: { title: string; content: string }) {
  return (
    <section className="border-t border-slate-100 pt-3.5 dark:border-slate-900">
      <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">{title}</h3>
      <div className={`mt-1.5 ${EXAM_ANSWER_TEXT_CLASS}`}>
...
export function ExamQuestionAnalysisSheet({ item }: ExamQuestionAnalysisSheetProps) {
  if (!item) {
    return (
      <aside className="relative min-h-[480px] w-full min-w-0 overflow-hidden border border-slate-200 bg-white px-6 py-6 shadow-[0_26px_70px_rgba(15,23,42,0.12)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-[0_28px_70px_-34px_rgba(0,0,0,0.9)] sm:px-8">
        <h2 className="font-sans text-xl font-bold text-slate-950 dark:text-slate-100">题目解析</h2>
        <div className="mt-6 rounded-xl border border-dashed border-slate-200 px-5 py-8 text-center text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400">
          请选择左侧题目查看解析。
        </div>
      </aside>
    );
  ...
  return (
    <aside className="relative min-h-[520px] w-full min-w-0 overflow-hidden border border-slate-200 bg-white px-5 py-5 shadow-[0_26px_70px_rgba(15,23,42,0.12)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-[0_28px_70px_-34px_rgba(0,0,0,0.9)] sm:px-6 lg:sticky lg:top-24 lg:max-h-[calc(100dvh-7rem)] lg:overflow-y-auto">
      <header className="border-b border-slate-100 pb-4 dark:border-slate-900">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">第 {item.item_order} 题</p>
            <h2 className="mt-1 font-sans text-xl font-bold text-slate-950 dark:text-slate-100">题目解析</h2>
          </div>
          <span
      ...
      <div className="mt-5 space-y-4.5">
        <section>
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">题干</h3>
          <div className={`mt-1.5 ${EXAM_QUESTION_TEXT_CLASS}`}>
