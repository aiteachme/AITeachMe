            ...
            <CardContent className="space-y-6">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="block text-sm text-slate-700">
                  当前形态
                  <div className="mt-1.5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,0.95fr)]">
                <label className="block text-sm text-slate-700">
                  ...
                  <textarea
                    rows={5}
                    className="mt-1.5 w-full resize-none rounded-[1.75rem] border border-slate-200 bg-white px-4 py-3 text-sm"
                    placeholder={
                      activeTab === "exam"
                        ? "例如：更像期末闭卷考试，覆盖网络分层、地址转换和操作系统基础，整卷层次清楚。"
                        : "例如：更偏日常练习，多考最近总错的知识点，少出纯记忆题。"
                    }
                    value={userPrompt}
                    disabled={
                      generateMutation.isPending ||
                ...
                <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50/70 p-4">
                  ...
                  <div className="mt-3 rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm text-slate-600">
                    <div className="font-medium text-slate-900">
                      当前选择：{selectedDifficulty?.label ?? "自动"}
                    </div>
              ...
              {activeTab === "exam" ? (
                <div className="space-y-4 rounded-[1.75rem] border border-stone-200 bg-[#fffaf2] p-5">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="block text-sm text-slate-700">
                      ...
                      <textarea
                        rows={4}
                        className="mt-1.5 w-full resize-none rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm"
                        placeholder="例如：更像学校闭卷试卷，题干完整，分段清晰，少教学性提示。"
                        value={stylePrompt}
                        disabled={
                          generateMutation.isPending ||
                          uploadSamplesMutation.isPending ||
                    ...
                    <label className="block text-sm text-slate-700">
                      ...
                      <textarea
                        rows={4}
                        className="mt-1.5 w-full resize-none rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm"
                        placeholder="例如：重点考协议分层、地址转换和系统调用等综合理解。"
                        value={focusPrompt}
                        disabled={
                          generateMutation.isPending ||
                          uploadSamplesMutation.isPending ||
                  ...
                  <div className="space-y-3 rounded-2xl border border-dashed border-stone-300 bg-white/80 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-slate-500">
                          如果没有样卷，系统也会自动按正式考卷风格生成；有样卷时会进一步参考其结构与题型习惯。
                        </div>
                      </div>
                  ...
                  <div className="mt-4 space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      <label className="block text-sm text-slate-700">
                        ...
                        <textarea
                          rows={4}
                          className="mt-1.5 w-full resize-none rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm"
                          placeholder="例如：题干更短、更像随堂测，减少冗长背景。"
                          value={stylePrompt}
                          disabled={
                            generateMutation.isPending ||
                            uploadSamplesMutation.isPending ||
                      ...
                      <label className="block text-sm text-slate-700">
                        ...
                        <textarea
                          rows={4}
                          className="mt-1.5 w-full resize-none rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm"
                          placeholder="例如：多考最近总错的知识点，少出纯定义题。"
                          value={focusPrompt}
                          disabled={
                            generateMutation.isPending ||
                            uploadSamplesMutation.isPending ||
                    ...
                    <div className="space-y-3 rounded-2xl border border-dashed border-slate-300 bg-white/80 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <div className="text-sm font-medium text-slate-900">
                            上传样卷（可选）
